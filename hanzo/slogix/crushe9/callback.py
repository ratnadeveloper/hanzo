...
from hanzo import YouTube, app
from hanzo.crushex.crushehitman import Hanzo
from hanzo.misc import SUDOERS, db
from hanzo.crushor.database import (
    get_active_chats,
    get_lang,
    get_upvote_count,
    is_active_chat,
...
from hanzo.crushor.database import get_assistant
from hanzo.crushor.decorators.language import languageCB
from hanzo.crushor.formatters import seconds_to_min
from hanzo.crushor.inline import close_markup, stream_markup
from hanzo.crushor.stream.autoclear import auto_clean
from config import (
    BANNED_USERS,
    SOUNCLOUD_IMG_URL,
    STREAM_IMG_URL,
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Hanzo.pause_stream(chat_id)
        ...
        await Hanzo.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        if command == "Skip":
            ...
            try:
                ...
                try:
                    ...
                    return await Hanzo.stop_stream(chat_id)
                except:
                    return
        else:
            txt = f"{mention}\n playing"
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Hanzo.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                ...
                image = None
            try:
                await Hanzo.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            button = stream_markup(_, chat_id)
            run = await CallbackQuery.message.reply_text(
            ...
            try:
                await Hanzo.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup(_, chat_id)
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Hanzo.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            if videoid == "telegram":
                button = stream_markup(_, chat_id)
